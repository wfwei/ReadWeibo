...
class UserClassifer():
		...
		for friend in self.user.friends.all():
			...
			for word in jieba.cut(desc.lower()):
				if word in self.keyword_set:
					logging.info('Predicting %s belongs to %s' % (friend, self.category))
					friend.predict_category = self.category
					friend.save()
					cnt += 1
					break
	...
	def auto_label_weibo(self):
		...
		while True:
			wbs = self.user.ownweibo.filter(predict_category__exact=None)[offset:offset+count]
			if not wbs:
				break
			else:
			...
			for wb in wbs:
				...
				for word in jieba.cut(wb.text.lower()):
					if word in self.keyword_set:
						...
						wb.predict_category = self.category
						wb.save()
						break
			offset += count
		pass
